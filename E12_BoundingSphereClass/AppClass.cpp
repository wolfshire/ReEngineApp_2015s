...
<<<<<<< HEAD
	for (uint i = 0; i < nVertexCount; i++) {
		if (vertexList[i].x > v3Max.x) v3Max.x = vertexList[i].x;
		else if (vertexList[i].x < v3Min.x) v3Min.x = vertexList[i].x;
		if (vertexList[i].y > v3Max.y) v3Max.y = vertexList[i].y;
		else if (vertexList[i].y < v3Min.y) v3Min.y = vertexList[i].y;
		if (vertexList[i].z > v3Max.z) v3Max.z = vertexList[i].z;
		else if (vertexList[i].z < v3Min.z) v3Min.z = vertexList[i].z;
	}

	m_v3Center1 = (v3Max + v3Min) / 2.0f;

	float fRadius = glm::distance(m_v3Center1, v3Max);
